...
/*
  The pageframe allocation table specifies which page frames have been physically mapped (are in use) and which are not.
  pageframe[i] = TRUE (Already Allocated)
  pageframe[i] = FALSE (Not Allocated)
*/
...
void initPageTable(pcb_t *pcb, int processEntry){
  ...
  pcb[processEntry].T[0x700] = ((pte_t) (i) << 20) | 0x00C22; //Page 700 (Kernel & Program Image), Client Domain | All Permissions - Full Access
  ...
  int stackPageFrame = nextFreePageFrame();
